...
class FilmController extends Controller
{
    ...
    {
        $films = Film::published()->featured()->with(['director', 'genres', 'media'])->get();
        return response()->json($films);
    }

    public function byStatus(string $status)
    {
        $films = Film::published()->byStatus($status)->with(['director', 'genres', 'media'])->get();
        return response()->json($films);
    }
    ...
    public function show(string $slug)
    {
        $film = Film::published()
            ->where('slug', $slug)
            ->with([
                'director', 'producer', 'writer', 'cinematographer', 'editor', 'composer',
                'genres',
                'cast.person',
                'crew.person',
                'awards',
